...
| `weapon`     | block?  | `{ damage: { physical?, magical? }, actionCost, scale?, crit?, twoHanded?, onHit? }`. Damage values are **dice notation** (see below). `crit` is an optional flat crit chance (0..1) the weapon adds on top of the wielder's Perception-derived crit (mirrors a mob's `attack.crit`); a crit doubles the damage roll. `twoHanded: true` marks the weapon as filling both hands — it can't be worn alongside a `shield`-slot piece, and equipping either auto-unequips the other. |
