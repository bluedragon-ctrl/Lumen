...
## [Unreleased]
### Added
- **The Argent Delvings** (west of the Moss Terraces): a dwindling Umbral silver-and-glimmer mine, 1 transitional room + 15 rooms spanning depth 9–11. The deep-folk still work the calm, played-out upper galleries (a still-lit adit, a greased winch, a broken-off sorting shift), but fewer come each season and the dangerous lower runs have been conceded to the burrowers — difficulty ramps from the calm mouth to a Necropolis-grade flooded sink. Mine `silver-vein`s and a `glimmer-vein` throughout; the Miners' Camp is a safe recovery spot with an Umbral curing-bench and a shuttered (switched-off) glimmer-lamp.
  - **Three new metal/mining mobs.** *A silver-back wyrm* (heavy ore-crusted burrower — armour 6, slow telegraphed `2d6`), *a great tremor-mole* (a bigger, bolder digger that ambushes from the stone instead of fleeing light), and *a shining ooze* (a metal-eating creep that drops the silver it has digested).
  - **Two bosses.** **Oreback** (depth 10, gates the winze down) — a vast silver-back wyrm grown half-seam; an armour-wall fight (160 HP, armour 9, `3d6` **pierce 4** + crush). Drops **Oreback's plate** (epic body armour: armour 4, heavy). **The Brightmire** (depth 11, over the richest seam) — a whole drowned level dissolved into silver-laced slime; an attrition/summon fight (150 HP, splits off lesser oozes). Drops **a quicksilver tear** (rare reagent).
- **The full dense-chitin line — the deep's heaviest armour, mirrored from the chitin set.** Five new rare pieces upgrade the `book-of-chitin-craft` line into black, mineral-banded dense plate, each recipe taking a distinct binding reagent (smithing bench): **a dense chitin helm** (armour 3; `silver-bar`), **a dense chitin warplate** (armour 5, the new armour ceiling — the dense mirror of heavy chitin plate; a whole `crystal`), **a dense chitin maul** (`2d8`, pierce 3; salt-quenched with `spring-salt`), **spiked dense chitin** (armour 4, `1d6` spikes; barbs set in `slug-slime`), and **a dense chitin shield** (armour 2, quicker than the tower-board; `glimmer-dust` at the boss). Taught by a new **`book of dense chitin craft`** (rare) — the true parallel to the chitin book — which teaches the whole dense line at once (the new five plus the existing dense cuirass and tower-shield), sold by the **Umbral stonewright**. The standalone `schematic-dense-chitin-cuirass` is retired in favour of the book.
...
- **The Umbral village qhatuq is now a working store, and the deep-folk keep a cold cuisine.** The market qhatuq (depth 10) sells the raw deep larder — oil, bug-tallow, palecap, witchglow cap, luminescent gland and blind cave-fish — plus a new **coldglow lamp** and the whole Umbral kitchen.
  ...
  - **Village shop tidy-up** — the village stonewright now sells the glimmerglass schematic and the book of dense chitin craft, and the layqa sells the Scroll of Glimmer Storm (all moved off Mallki, where they sat as placeholders).
...
- **`attributes` (`attr` / `stats`) — a live character sheet that explains what
  ...
  bleed soak, Intellect → max MP, Perception → to-hit/crit, Wits → spellward +
  evasion. Weapon damage is weapon-aware — the "+N damage" clause hangs on whichever
  attribute your held weapon actually scales on (a glimmer blade reads on Intellect,
  not Might), and Might reads "none equipped right now" when your weapon ignores it.
  Vitality also states the physical-DoT soak it grants (`floor(Vit/8)` per tick),
  0 at baseline like Armour. A **Defences** block then spells out all four mitigation
  pools in plain language — **Armour** (flat cut to physical), **Spellward** (chance
  to fizzle a hostile spell *or a magical DoT tick* + a percent cut to magical weapon
  blows), **Voidward** (the same vs void — casts, ticks & blows), and **Evasion**
  (chance to dodge a blow). Gear/buff shifts annotate as
...
- **3D world map viewer (`tools/map-3d/`).** A local, read-only, browser-based 3D model of the
  ...
  double-click `tools/map-3d/start.bat`, port 3945, `MAP_3D_PORT` overrides) serves it and
  re-reads `rooms.json` on each reload. Dependency-free (a tiny vanilla `<canvas>` 3D engine —
